...
namespace Sudoku
{
    ...
    private int attempts = 0;
		...
		{
      SudokuOOP sudoku = new SudokuOOP();

      sudoku.shuffleFirstRow();
      sudoku.generateSudokuBoard(sudoku.sudokuBoard);
      Console.WriteLine("\nProgram took " + sudoku.attempts + " attempts to generate sudoku board:\n");
      sudoku.printBoard();
      Console.WriteLine("\nValid Board: " + sudoku.isValidSudokuBoard(sudoku.sudokuBoard));
		}
    ...
    /// </returns>
    private bool validateCoordinate(int[,] currentBoardState, int number, int[] coordinates) 
    {
    ...
    private bool printBoard() 
    {
      ...
      for(int yCoordinate = 0; yCoordinate < 9; yCoordinate++) {
        ...
        for(int xCoordinate = 0; xCoordinate < 9; xCoordinate++) {
          sudokuBoard += (" " + this.sudokuBoard[yCoordinate, xCoordinate] + " ");
    ...
    /// </returns>
    private int[] getEmptySpotCoordinate(int[,] currentBoardState) 
    {
      for(int yCoordinate = 0; yCoordinate < 9; yCoordinate++)
      {
        for(int xCoordinate = 0; xCoordinate < 9; xCoordinate++)
    ...
    ///   A boolean value.
    /// </returns>
    private bool isValidSudokuBoard(int[,] board) 
    {
